...
`SafeStateProcedureRef` is exact `{board_id,facts_handoff,assertion_ids,procedure}`. It binds the procedure to one physical device identity and to verified assertions in a ready `02-facts` handoff. That binding is checked, not merely pinned: the helper parses `facts_handoff`, requires stage `extract-facts`, status `ready` and a `passed` `citations-verified` check, and resolves every `assertion_ids` entry against the citations it actually carries. A hash pin alone would prove only that some bytes did not change, not that the file is a handoff at all. It does not prove the procedure is physically sufficient, and it does not prove the cited assertions entail that this board is safe; that judgement is review's.
