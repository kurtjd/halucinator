...
Required: `driver.name`; `scope_kind=full|scaffold-support`; `owned_files` ArtifactRef[]; `capabilities`; complete tester-safe `public_api`; structured `dependencies`; `trait_obligations` exact `{dependency_crate,trait,obligations:string[]}[]`; `test_hardware_facts` CitationRef[]; `build_contract` exact `{cargo_chip_feature,rust_compilation_target,init_calls:string[],memory_runtime:FileRef,observation}`; `requirement_ids`; optional `public_test_record` FileRef. Sources: `gpio-record.md:13-61`, `time-record.md:8-54`; consumers `gpio-validation.md:9-20`, `time-validation.md:8-23`.
